...
public class RequestHandler implements Runnable
{
   ...
   public RequestHandler(Socket clientSocket)
   {
      ...
      try
      {
         ...
         proxyToClientBr = new BufferedReader(
               new InputStreamReader(clientSocket.getInputStream()));
         proxyToClientBw = new BufferedWriter(
   ...
   private void sendCachedPageToClient(File cachedFile)
   {
      ...
      try
      {
         ...
         String fileExtension = cachedFile.getName()
               .substring(cachedFile.getName().lastIndexOf('.'));
               ...
               || fileExtension.contains(".gif"))
         {
            ...
            {
               System.out
                     .println("Image " + cachedFile.getName() + " was null");
   ...
   private void sendNonCachedToClient(String urlString)
   {
      ...
      try
      {
         ...
         // This allows the files on stored on disk to resemble that of the URL
         // it was taken from
         ...
         fileExtension =
               urlString.substring(fileExtensionIndex, urlString.length());
   ...
   private void handleHTTPSRequest(String urlString)
   {
      ...
      try
      {
         // Only first line of HTTPS request has been read at this point
         // (CONNECT *)
         ...
         // Client and Remote will both start sending data to proxy at this
         // point
         // Proxy needs to asynchronously read data from each party and send it
         // to the other party
         ...
         ClientToServerHttpsTransmit clientToServerHttps =
               new ClientToServerHttpsTransmit(clientSocket.getInputStream(),
   ...
   private void blockedSiteRequested()
   {
      ...
      {
         System.out.println(
               "Error writing to client when requested a blocked site");
         e.printStackTrace();
      }
   }
}